...
function calcRoute() {
  ...
  directionsService.route(request, function(response, status) {
    if (status == google.maps.DirectionsStatus.OK) {
      ...
      var pos = new google.maps.LatLng(43.1861815, -77.616399);
      ...
      // **add call to icons function embedded in a timeout**
      ...
      $.getJSON(url, opts).done(function(data) {
        try {
          if(day === "today") {
            ...
            icons.push(data.currently.icon);
          } else {
...
// display weather icon at each weather read along polyline
  // pull icon data from each weather read from forecast.io and use it to select appropriate svg path
  // take latlngs from each weather read and create a marker at that position using the svg icon
...
// display weather summary text in text box on weather icon hover
